    ...
    public void applyProperty(View hostview, String key, String value, Object modelData) {
        ...
        switch (key) {
            ...
            case "padding": {
                ...
                if (split.length == 1) {
                    String s = split[0];
                    paddingleft = paddingRight = paddingTop = paddingBottom = (int) XNodeUtil.meatureWithUnit(s);
                } else if (split.length == 4) {
                    paddingleft = (int) XNodeUtil.meatureWithUnit(split[0]);
                    paddingRight = (int) XNodeUtil.meatureWithUnit(split[2]);
                    paddingTop = (int) XNodeUtil.meatureWithUnit(split[1]);
                    paddingBottom = (int) XNodeUtil.meatureWithUnit(split[3]);
                }

                hostview.setPadding(paddingleft, paddingRight, paddingTop, paddingBottom);
                break;
            }
